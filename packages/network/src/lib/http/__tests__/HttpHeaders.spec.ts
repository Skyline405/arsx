...
describe('class HttpHeaders', () => {
  ...
  describe('should be created via constructor', () => {
    ...
    it('with array of pairs of strings', () => {
      ...
      expect(instance.get('b')).toEqual('2')
      expect(instance.getAll('a')).toEqual(['1'])
      expect(instance.getAll('b')).toEqual(['2', '3'])
    })

    it('with another HttpHeaders', () => {
      const instance = new HttpHeaders(new HttpHeaders(headersRecord))
      expect(instance.getAll('a')).toEqual(headersRecord['a'])
      expect(instance.getAll('b')).toEqual(headersRecord['b'])
      expect(instance.getAll('c')).toEqual(headersRecord['c'])
    })
  })
  ...
  describe('append() method', () => {
    it('should add value to the values list', () => {
      ...
      expect(instance.getAll('a')).toEqual(['a1'])

      instance.append('a', 'a2')
      expect(instance.getAll('a')).toEqual(['a1', 'a2'])

      instance.append('a', ['a2', 'a1']) // try to add existing values
      expect(instance.getAll('a')).toEqual(['a1', 'a2', 'a2', 'a1'])
    })
    ...
    it('should add value from comma separated string', () => {
      ...
      expect(headers.getAll('Accept')).toEqual(['application/json', 'text/html; charset=utf-8'])
    })
  ...
  describe('set() method', () => {
    it('should add values', () => {
      ...
      expect(headers.getAll('a')).toEqual(['some', 'values'])
    })

    it('should override values', () => {
      headers.set('a', 'some')
      headers.set('a', ['values'])
      expect(headers.getAll('a')).toEqual(['values'])
    })
  })

  describe('get() method', () => {
    it('should return value by case insensetive key', () => {
      expect(headers.getAll('A')).toEqual(['a1', 'a2'])
    })
  })
})
